...
function chemical_mpo(
    ...
    symbolic_mpo, virt_spaces =
        construct_symbolic_mpo(op_terms, symm_ctx; algo = algo, verbose = verbose)
    ...
    if uppercase(backend) == "TENSORKIT"
        ...
        throw(
            ArgumentError(
                "Unsupported backend: $backend. Supported backends are: 'TensorKit'.",
            ),
        )
    end
...
chemical_mpo(op_terms::ChemOpSum, symm::String; kwargs...) =
    chemical_mpo(op_terms, create_symmetry_context(symm); kwargs...)
...
chemical_mpo(molecule::Molecule; kwargs...) =
    chemical_mpo(xyz_string(Molecule(molecule)); kwargs...)
...
chemical_mpo(
    ...
    nuc_e::Float64;
    kwargs...,
) = chemical_mpo(h1e, h2e; nuc_e = nuc_e, kwargs...)
