...
function dmrg(
    mps::Vector{BlockTensorMap},
    H_mpo::Vector{SparseBlockTensorMap};
    dmrg_sweeps::Int = 10,
    mps_maxdim::Int = 32,
    truncErr::Float64 = 1e-8,
    verbose::Bool = false,
    ...
    mps, gsE = find_groundstate_wMaxdim(
        mps,
        H_mpo,
        DMRGCONFIG(;
            bondDim = mps_maxdim,
...
function find_groundstate_wMaxdim(
    finiteMPS::Vector{BlockTensorMap},
    finiteMPO::Vector{SparseBlockTensorMap},
    alg::DMRGCONFIG,
)
...
function find_groundstate_wMaxdim!(
    ...
    truncMethod =
        alg.truncErr > 0.0 ? truncdim(alg.bondDim) & truncerr(alg.truncErr) :
    ...
    if alg.subspaceExpansion
        finiteMPS =
            applyMPO(finiteMPO, finiteMPS; maxDim = alg.bondDim, truncErr = alg.truncErr) # TO IMPLEMENT: split maxDim and truncErr between applyMPO and find_groundstate # compressionAlg = "zipUp")
    end
    ...
    while runOptimization
        ...
        mpoEnvL, mpoEnvR =
            initializeMPOEnvironments(finiteMPS, finiteMPO; dataType = dataType)
        ...
        while runOptimizationDMRG
            ...
            for siteIdx = 1:+1:(length(finiteMPS)-1)
                ...
                eigenVal, eigenVec = eigsolve(
                    theta,
                    1,
                    :SR,
                    KrylovKit.Lanczos(; tol = alg.eigsTol, maxiter = alg.maxIterations),
                ) do x
                ...
                # update mpoEnvL
                mpoEnvL[siteIdx+1] = SparseBlockTensorMap(
                    update_MPOEnvL(
                        mpoEnvL[siteIdx],
                        finiteMPS[siteIdx],
                        finiteMPO[siteIdx],
            ...
            for siteIdx = (length(finiteMPS)-1):-1:1
                ...
                # update mpoEnvR
                mpoEnvR[siteIdx+0] = SparseBlockTensorMap(
                    update_MPOEnvR(
                        mpoEnvR[siteIdx+1],
                        finiteMPS[siteIdx+1],
                        finiteMPO[siteIdx+1],
            ...
            if abs(imag(mpoExpVal)) < 1e-12
                ...
                ErrorException(
                    "the Hamiltonian is not Hermitian, complex eigenvalue found.",
                )
            end
            ...
            alg.verbose > 0 && println(
                "DMRG step $(loopCounter) ; energy = $(round(mpoExpVal, digits=8)) ; convergence = $(round(energyConvergence, digits=8))",
            )
        ...
        alg.verbose &&
            alg.verbose &&
            println(
                "Energy variance ⟨ψ|(H - E)^2|ψ⟩ = $(round(energyVariance, digits=4, sigdigits=4))",
            )
...
function initializeMPOEnvironments(
    finiteMPS::Vector{BlockTensorMap},
    finiteMPO::Vector{SparseBlockTensorMap};
    centerPos::Int64 = 1,
    dataType::DataType = Float64,
    ...
    mpoEnvL[1] = SparseBlockTensorMap(
        ones(
            dataType,
            space(finiteMPS[1], 1),
    ...
    mpoEnvR[N] = SparseBlockTensorMap(
        ones(
            dataType,
    ...
    for siteIdx = N:-1:(centerPos+1)
        mpoEnvR[siteIdx-1] = SparseBlockTensorMap(
            update_MPOEnvR(
                mpoEnvR[siteIdx],
                finiteMPS[siteIdx],
                finiteMPO[siteIdx],
...
function dotMPS(mpsA::Vector{BlockTensorMap}, mpsB::Vector{BlockTensorMap})
    ...
    N = length(mpsA)
    N != length(mpsB) && throw(
        DimensionMismatch("lengths of MPS A ($N) and MPS B ($length(mpsB)) do not match"),
    )

    # initialize overlap
    overlapMPS = ones(Float64, space(mpsA[1], 1), space(mpsB[1], 1))
    for siteIdx = 1:N
        @tensor overlapMPS[-1; -2] :=
            overlapMPS[1, 2] * conj(mpsA[siteIdx][1, 3, -1]) * mpsB[siteIdx][2, 3, -2]
    end
...
function applyMPO(
    ...
    isomoL = isomorphism(
        fuse(space(compressedMPS[1], 1), space(finiteMPO[1], 1)),
    ...
    for siteIdx = 1:N
        if siteIdx < N
            ...
            @tensor compressed_mps[-1 -2; -3] :=
                isomoL[-1, 1, 3] *
                compressedMPS[siteIdx][1, 2, 4] *
                finiteMPO[siteIdx][3, -2, 5, 2] *
                isomoR[4, 5, -3]
            compressedMPS[siteIdx] = BlockTensorMap(compressed_mps)
        end
    end
...
function orthogonalizeMPS!(mps::Vector{BlockTensorMap})
    for siteIdx = length(mps):-1:1
        (L, Q) = rightorth(mps[siteIdx], ((1,), (2, 3)); alg = LQpos())
        # normalizeMPS && normalize!(L)
        if siteIdx > 1
            mps[siteIdx-1] = BlockTensorMap(
                TensorKit.permute(
                    TensorKit.permute(mps[siteIdx-1], ((1, 2), (3,))) * L,
                    ((1, 2), (3,)),
                ),
            )
            mps[siteIdx-0] = BlockTensorMap(TensorKit.permute(Q, ((1, 2), (3,))))
        else
            mps[siteIdx-0] = BlockTensorMap(
                TensorKit.permute(L * TensorKit.permute(Q, ((1,), (2, 3))), ((1, 2), (3,))),
            )
        end
    end
end
...
function Base.:+(mpsA::Vector{BlockTensorMap}, mpsB::Vector{BlockTensorMap})
    ...
    if NA == 1
        ...
        @tensor newTensor[-1 -2; -3] :=
            mpsA[idxMPS][-1, -2, 3] * isoRA[3, -3] + mpsB[idxMPS][-1, -2, 3] * isoRB[3, -3]
        ...
        for idxMPS = 2:(NA-1)
            ...
            isoRB = rightnull(isoRA)
            @tensor newTensor[-1 -2; -3] :=
                isoLA[-1, 1] * mpsA[idxMPS][1, -2, 3] * isoRA[3, -3] +
                isoLB[-1, 1] * mpsB[idxMPS][1, -2, 3] * isoRB[3, -3]
            MPSC[idxMPS] = BlockTensorMap(newTensor)
        end
        ...
        isoLB = leftnull(isoLA)
        @tensor newTensor[-1 -2; -3] :=
            isoLA[-1, 1] * mpsA[idxMPS][1, -2, -3] + isoLB[-1, 1] * mpsB[idxMPS][1, -2, -3]
        MPSC[idxMPS] = BlockTensorMap(newTensor)
    end
    return MPSC
end
...
function applyH2(X, EL, mpo1, mpo2, ER)
    @tensor X[-1 -2; -3 -4] :=
        EL[-1, 2, 1] * X[1, 3, 5, 6] * mpo1[2, -2, 4, 3] * mpo2[4, -3, 7, 5] * ER[6, 7, -4]
    return X
end
...
function update_MPSEnvR(mpsEnvR, mpsTensorK, mpsTensorB)
    @tensor newER[-1; -2] :=
        mpsTensorK[-1, 2, 1] * conj(mpsTensorB[-2, 2, 3]) * mpsEnvR[1, 3]
    return newER
end
