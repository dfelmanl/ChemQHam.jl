...
function init_mps_hf(
    occHF_orbBasis,
    occHF_elCnt;
    symm::String = "U1SU2",
    dataType::DataType = Float64,
)::Vector{BlockTensorMap}
    ...
    if length(occHF_orbBasis) != length(occHF_elCnt)
        error(
            "Length mismatch: occHF_orbBasis ($(length(occHF_orbBasis))) and occHF_elCnt ($(length(occHF_elCnt))) must have the same length (number of spatial orbitals)",
        )
    end
    ...
    for siteIdx = 2:(N_spt+1)
        cummulative_n_el = sum(occHF_elCnt[1:(siteIdx-1)])

        if uppercase(symm) == "U1U1"
            fParity = cummulative_n_el % 2 == 0 ? 0 : 1
            virtSpaces[siteIdx] = SumSpace(
                Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[U₁])]((fParity, cummulative_n_el) => 1),
            )
        elseif uppercase(symm) == "U1SU2"
            ...
            virtSpaces[siteIdx] = SumSpace(
                Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])](
                    (fParity, cummulative_n_el, spin) => 1,
                ),
            )
        else
    ...
    for siteIdx = 1:N_spt
        ...
        ftree_tm = TensorMap([1.0], space(btm[idxs]))
...
function init_mps_rand(
    ...
    if uppercase(symm) == "U1U1"
        boundarySpaceR =
            SumSpace(Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[U₁])]((N_el % 2, N_el, spin) => 1))
    elseif uppercase(symm) == "U1SU2"
        boundarySpaceR = SumSpace(
            Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])]((N_el % 2, N_el, spin) => 1),
        )
    else
        error("Symmetry type $symm not supported")
    end
    ...
    initialTensors = Vector{BlockTensorMap}(undef, N_spt);
    for siteIdx = 1:N_spt
        initialTensors[siteIdx] =
            randn(dataType, virtSpaces[siteIdx] ⊗ phy_sumspace, virtSpaces[siteIdx+1])
    end
...
function constructVirtSpaces(
    ...
    virt_spaces_gs = constructVirtSpaces(
        phySpaces,
        N_spt,
        boundarySpaceL_gs,
        boundarySpaceR_gs;
...
function removeDegeneracyQN(vecSpace; degenCutOff::Int64 = 1)
    ...
    truncatedVectorSpace = typeof(vecSpace)([
        key => min(qnSectors[key], degenCutOff) for key in keys(qnSectors)
    ])
