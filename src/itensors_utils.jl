...
function ITChem_opsum(
    ...
    ham_info = molecular_orbital_hamiltonian(
        molecule;
        basis = "sto-3g",
        atol = ops_tol,
        n_sites = n_sites,
...
function ITChem_mat(opsum; n_sites = 0, ham_tol = 1e-10, ham_maxdim = 10000)
    n_sites =
        n_sites > 0 ? n_sites :
        maximum(
            site_op.sites[1] for op_term in opsum.args[1] for site_op in op_term.args[2]
        )
    ...
    H_sparse =
        SparseArrays.sparse(reshape(Array(H_tens, mpo_sites), (4^n_sites, 4^n_sites)))
    return H_sparse
end
