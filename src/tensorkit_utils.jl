...
function construct_empty_sparse_block_site(
    phySumSpace::SumSpace,
    left_vs::Vector{QN},
    right_vs::Vector{QN},
    symm::String;
    ...
    if symm == "U1SU2"
        ...
        sum_Vspace_left =
            ⊕([
                Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])](vs => 1) for
                vs in first.(left_vs)
            ]...)
        sum_Vspace_right =
            ⊕([
                Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])](vs => 1) for
                vs in first.(right_vs)
            ]...)
    elseif symm == "U1U1"
        # For U1U1 symmetry, we use FermionParity ⊠ U1Irrep ⊠ U1Irrep
        sum_Vspace_left =
            ⊕([Vect[(FermionParity⊠U1Irrep⊠U1Irrep)](vs => 1) for vs in first.(left_vs)]...)
        sum_Vspace_right =
            ⊕([Vect[(FermionParity⊠U1Irrep⊠U1Irrep)](vs => 1) for vs in first.(right_vs)]...)
    else
        throw(
            ArgumentError(
                "Unsupported symmetry type: $symm. Supported symmetries are 'U1SU2' and 'U1U1'.",
            ),
        )
    end
    ...
    for (row, col, ops) in zip(findnz(symb_mpo_site)...)
        ...
        for (op_str, op_coef) in ops
            ...
            for (ftree_left, ftree_right, val) in op_data
                ...
                old_val =
                    only(ftree_tm[ftree_type(ftree_left...), ftree_type(ftree_right...)])
                new_val = old_val + val * op_coef
                ftree_tm[ftree_type(ftree_left...), ftree_type(ftree_right...)] .= new_val

                verbose && println(
                    "Filling MPO site for op_str: $op_str, left QN: $left_vsQN, left_op_mult: $left_vsMult, right QN: $right_qnQN, right_op_mult: $right_vsMult",
                )
                verbose && println(
                    "sp_coordinates: $sp_coordinates, old_val: $old_val, new_val: $new_val, CG coef val: $val, operator's coef: $op_coef",
                )
    ...
    for (row, col, ops) in zip(findnz(symb_mpo_site)...)
        ...
        for op in ops
            ...
            for (ftree_left, ftree_right, val) in op_data
                ...
                verbose && println(
                    "Filling MPO site for operator: $op, op_str $op_str, left QN: $left_qn, right QN: $right_qn, left_mult: $left_mult, right_mult: $right_mult, ftree_left: $ftree_left, ftree_right: $ftree_right, value: $val, coef: $(op.coefficient)",
                )
...
function symbolic_to_tensorkit_mpo(
    symbolic_mpo,
    virt_spaces,
    symm_ctx::AbstractSymmetryContext;
    ...
    for (isite, symb_mpo_site) in enumerate(symbolic_mpo)
        ...
        sblock_site = construct_empty_sparse_block_site(
            phySumSpace,
            vs_left,
            vs_right,
            symm_ctx.name;
...
function mpo_to_mat(H_mpo)
    ...
    for i = 1:N_spt
        ...
        H_contraction = TensorKit.permute(
            H_contraction * TensorKit.permute(H_mpo[i], (1,), (2, 3, 4)),
            domain_legs,
            codomain_leg,
        )
    end
    ...
    mpo_mat =
        SparseArrays.sparse(reshape(convert(Array, H_contraction), (4^N_spt, 4^N_spt)))
    return mpo_mat
end
...
function genFlattenedPhySpace(symm)
    ...
    if uppercase(symm) == "U1"
        phySumSpace =
            Vect[(FermionParity⊠Irrep[U₁])]((0, 0) => 1) ⊕
            Vect[(FermionParity⊠Irrep[U₁])]((1, 1) => 1) ⊕
            Vect[(FermionParity⊠Irrep[U₁])]((1, 1) => 1) ⊕
            Vect[(FermionParity⊠Irrep[U₁])]((0, 2) => 1)
    elseif uppercase(symm) == "U1U1"
        # (fParity, total count, spin count)
        phySumSpace =
            Vect[(FermionParity⊠U1Irrep⊠U1Irrep)]((0, 0, 0) => 1) ⊕
            Vect[(FermionParity⊠U1Irrep⊠U1Irrep)]((1, 1, 1 // 2) => 1) ⊕
            Vect[(FermionParity⊠U1Irrep⊠U1Irrep)]((1, 1, -1 // 2) => 1) ⊕
            Vect[(FermionParity⊠U1Irrep⊠U1Irrep)]((0, 2, 0) => 1)
    elseif uppercase(symm) == "U1SU2"
        phySumSpace =
            Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])]((0, 0, 0) => 1) ⊕
            Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])]((1, 1, 1 // 2) => 1) ⊕
            Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])]((0, 2, 0) => 1)
    else
        throw(
            ArgumentError(
...
function get_physical_abs_offsets(symm::String)
    ...
    physical_offsets = Dict(
        ftree_inner_data(phySumSpace[i].dims.keys[1]) => i for i = 1:length(phySumSpace)
    )

    return physical_offsets
end

function merge_site_physical_idx(
    mpo_site::SparseBlockTensorMap;
    clockwise_codomain::Bool = false,
)
    iso_dom = isometry(
        ⊕(Vect[(FermionParity⊠Irrep[U₁]⊠Irrep[SU₂])](
            (0, 0, 0)=>1,
            (1, 1, 1/2)=>1,
            (0, 2, 0)=>1,
        )),
    ...
    if clockwise_codomain
        @tensor mpo_site_f_iso[-1, -2; -3, -4] :=
            iso_dom[-2, 1] * mpo_site[-1, 1, 2, -4] * iso_codom[-3, 2]
    else
        @tensor mpo_site_f_iso[-1, -2; -3, -4] :=
            iso_dom[-2, 1] * mpo_site[-1, 1, -3, 2] * iso_codom[-4, 2]
    end
...
function merge_physical_idx(
    mpo::Vector{<:SparseBlockTensorMap};
    clockwise_codomain::Bool = false,
)
    merger(site::SparseBlockTensorMap) =
        merge_site_physical_idx(site; clockwise_codomain = clockwise_codomain)
    return merger.(mpo)
end
...
flip_incoming_indices(mpo_site::SparseBlockTensorMap) =
    TensorKit.permute(mpo_site, (1, 2), (4, 3))
